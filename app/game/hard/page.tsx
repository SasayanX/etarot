...
// カードコンポーネントをメモ化
const Card = memo(
  ({ card, index, handleCardClick, cardBackImage, currentRound, isWebView, handleImageError, isShuffling }: any) => {
    return (
      <div
        key={card.id}
        className={cn(
          ...
          isShuffling && !card.isMatched ? "card-shuffling" : "",
        )}
      ...
      >
        <div className="card-inner">
          ...
          <div className="card-front shadow-md">
            <img
              src={card.image || "/placeholder.svg?height=300&width=200&text=Loading..."}
              alt={card.name}
...
export default function HardGamePage() {
  ...
  useEffect(() => {
    ...
    console.log("Hard mode: Initial card back:", currentCardBack)
    ...
    const handleCardSuitChange = (event: Event) => {
      try {
        ...
        if (customEvent.detail && customEvent.detail.imageUrl) {
          console.log("Hard mode: Card suit changed event received:", customEvent.detail)
    ...
    const refreshInterval = setInterval(() => {
      try {
        ...
        if (latestCardBack !== cardBackImage) {
          console.log("Hard mode: Refreshing card back:", latestCardBack)
  ...
  // ハードモード用の状態変数
  // visibleCardCount、maxVisibleCards関連の状態変数と処理を削除
  // const [visibleCardCount, setVisibleCardCount] = useState<number>(0)
  // const [maxVisibleCards, setMaxVisibleCards] = useState<number>(3) // 最大3枚まで同時に表示可能
  ...
  /* シャッフルアニメーション */
  .card-shuffling {
    animation: shuffleCard 0.8s cubic-bezier(0.4, 0, 0.2, 1);
    z-index: 10;
  }
  ...
  @keyframes shuffleCard {
    ...
    25% { 
      transform: scale(0.8) rotateZ(-5deg) translateY(-10px);
      opacity: 0.8;
    }
    50% { 
      transform: scale(0.6) rotateZ(5deg) translateY(-20px);
      opacity: 0.6;
    }
    75% { 
      transform: scale(0.8) rotateZ(-2deg) translateY(-10px);
      opacity: 0.8;
    }
    100% { 
...
`
  ...
  const handleCardClick = useCallback(
    (index: number) => {
      ...
      if (
        ...
        flippedCards.length >= 2 // 2枚以上めくれないように制限
      ) {
          ...
          if (firstCard.cardId === secondCard.cardId) {
            ...
            setMatchedPairs((prevMatchedPairs) => {
              ...
              if (newMatchedPairs >= 8) {
                console.log("All pairs found, setting victory flags")
            ...
            setTimeout(() => {
              setCards((prevCards) =>
                prevCards.map((card, i) =>
                  (i === firstIndex || i === secondIndex) && !card.isMatched ? { ...card, isFlipped: false } : card,
                ),
              )
              setFlippedCards([])
  ...
  const shuffleCards = useCallback(() => {
    ...
    // 現在のカードから、マッチしていないカードだけを取得
    const unmatchedCards = cards.filter((card) => !card.isMatched)

    // マッチしていないカードをシャッフル
    const shuffledUnmatched = [...unmatchedCards].sort(() => 0.5 - Math.random())
    ...
    cards.forEach((card, index) => {
      if (!card.isMatched) {
        unmatchedPositions.push(index)
      }
    })

    // シャッフルされたカードを新しい位置に配置
    shuffledUnmatched.forEach((card, idx) => {
      const newPosition = unmatchedPositions[idx]
      if (newPosition !== undefined) {
        // 重要: 画像情報を確実に保持
        newCards[newPosition] = {
          ...card,
          id: newPosition,
          isFlipped: false, // すべてのカードを裏返す
          // 画像関連の情報を明示的に保持
          image: card.image,
    ...
    // シャッフル完了後、少し遅延してからクリック可能にする
    setTimeout(() => {
      setIsShuffling(false)
    }, 800) // アニメーション時間を考慮して少し長めに
  ...
  const handleCardInfoDialogClose = useCallback(() => {
    ...
    if (showVictoryAfterCardInfo) {
      ...
      setTimeout(() => {
        ...
        if (remainingCards > 0) {
          console.log(`ペア獲得後 - 残りカード${remainingCards}枚をシャッフル`)
          shuffleCards()
        }
      }, 500)
    }
  ...
  const handleSaveScore = useCallback(
    (playerName?: string) => {
      try {
        ...
        // スコアを記録
        const wasAdded = addScore("hard", score, moves, finalPlayerName, finalTarotPoints)

        // タロットポイントを確実に加算
        addPoints(finalTarotPoints)
        console.log(`タロットポイント加算: +${finalTarotPoints}TP`)
    ...
    [score, moves, toast, router, addScore, addPoints],
  )
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-amber-400">ハードモード</h1>
