...
// 大アルカナの画像URLを取得する関数（辞典と同じ）
function getMajorArcanaImageUrl(cardId: number): string {
  const majorArcanaImages: Record<number, string> = {
  ...
  return (
    majorArcanaImages[cardId] || `/placeholder.svg?height=300&width=200&text=${encodeURIComponent(getCardName(cardId))}`
  )
}
...
export default function TimeAttackGamePage() {
  ...
  useEffect(() => {
    ...
    console.log("Time Attack mode: Initial card back:", currentCardBack)
    ...
    const handleCardSuitChange = (event: Event) => {
      try {
        ...
        if (customEvent.detail && customEvent.detail.imageUrl) {
          console.log("Time Attack mode: Card suit changed event received:", customEvent.detail)
    ...
    const refreshInterval = setInterval(() => {
      try {
        ...
        if (latestCardBack !== cardBackImage) {
          console.log("Time Attack mode: Refreshing card back:", latestCardBack)
  ...
  // タイムアタックモード用の状態変数
  const [timeLeft, setTimeLeft] = useState<number>(180) // 180秒（3分）
  ...
  }, [isTimerRunning, timeLeft, soundEnabled, playSound])
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 22; i++) {
      urls[i] = getMajorArcanaImageUrl(i)
    }
  ...
  const initializeGame = useCallback(() => {
    ...
    setTotalRounds(1)
    setTimeLeft(180) // タイムアタック用の制限時間をリセット（3分）
    setIsTimerRunning(false) // タイマーを停止
    setGameOver(false) // ゲームオーバーフラグをリセット
    ...
    if (includeMinorArcanaInGames) {
      ...
      try {
        ...
        const majorArcanaCards = Array.from({ length: 22 }, (_, i) => {
          const imageUrl = getMajorArcanaImageUrl(i)
          return {
            id: i,
            name: getCardName(i),
            image: imageUrl,
  ...
  // 最初のカードがめくられたらタイマーを開始
  useEffect(() => {
    if (flippedCards.length > 0 && !isTimerRunning && isGameInProgress && !gameOver) {
      setIsTimerRunning(true)
    }
  }, [flippedCards, isTimerRunning, isGameInProgress, gameOver])

  // Move generateNewCardSet function here (before startNewRound)
  const generateNewCardSet = useCallback(() => {
    // 設定に基づいてカードを選択
    let availableCards = []

    if (includeMinorArcanaInGames) {
      try {
        // 大アルカナカードを確実に取得（画像URL修正）
        const majorArcanaCards = Array.from({ length: 22 }, (_, i) => {
          const imageUrl = getMajorArcanaImageUrl(i)
          return {
            id: i,
  ...
  }, [includeMinorArcanaInGames, cardImageUrls, setCards])

  // Move startNewRound function here (before handleCardClick)
  const startNewRound = useCallback(() => {
    console.log("新しいラウンドを開始")
    ...
    // ラウンド開始の通知
    toast({
      title: `ラウンド ${currentRound + 1} 開始！`,
      description: "新しいカードセットが配られました",
      duration: 3000,
    })

    if (textToSpeechEnabled) {
      speak(`ラウンド${currentRound + 1}開始`)
    }
  }, [currentRound, toast, textToSpeechEnabled, speak, generateNewCardSet])
      ...
      if (newFlippedCards.length === 2) {
        ...
        setTimeout(() => {
          ...
          if (firstCard.cardId === secondCard.cardId) {
            ...
            console.log(`スコア計算: ${newCombo}回目, 基本スコア${baseScore}, ボーナス${comboBonus}, 合計${totalScore}`)

            setScore((prevScore) => {
              const newScore = prevScore + totalScore
              console.log(`スコア更新: ${prevScore} + ${totalScore} = ${newScore}`)
              return newScore
            })
  ...
  const handleImageError = useCallback(
    (e: React.SyntheticEvent<HTMLImageElement, Event>, card: { name: string; cardId: number }) => {
      ...
      console.error(`画像読み込みエラー: ${card.name} (ID: ${card.cardId})`)

      // 最終手段としてシンプルなプレースホルダーを使用
      target.src = `/placeholder.svg?height=300&width=200&text=${encodeURIComponent(card.name)}`
      target.onerror = null // これ以上のエラーを防ぐ
    },
    [],
  )
  ...
  const handleSaveScore = useCallback(
    (playerName?: string) => {
      try {
        ...
        console.log(`[TP DEBUG] スコア保存開始: ${score}点 -> ${finalTarotPoints}TP`)
        console.log(`[TP DEBUG] 現在のポイント: ${points}`)

        // ローカルストレージに直接保存して確実にポイントを加算
        if (typeof window !== "undefined") {
          const currentPoints = Number.parseInt(localStorage.getItem("tarotPoints") || "0", 10)
          const newPoints = currentPoints + finalTarotPoints

          console.log(`[TP DEBUG] ローカルストレージ直接更新: ${currentPoints} + ${finalTarotPoints} = ${newPoints}`)
          localStorage.setItem("tarotPoints", newPoints.toString())

          // カスタムイベントを発火してShopContextに通知
          const event = new CustomEvent("tarotPointsUpdated", {
            detail: { points: newPoints, added: finalTarotPoints },
          })
          window.dispatchEvent(event)
        }

        // addPoints関数も呼び出す（二重保険）
        addPoints(finalTarotPoints)

        // モード名を修正
        const wasAdded = addScore("timeAttack", score, moves, finalPlayerName, finalTarotPoints)
    ...
    [score, moves, toast, router, addPoints, addScore, points],
  )

  const handlePlayerNameDialogClose = (playerName?: string) => {
    setShowPlayerNameDialog(false)
    if (playerName) {
      handleSaveScore(playerName)
    } else {
      // スコア記録をスキップしてメニューに戻る
      setTimeout(() => {
        router.push("/game")
  ...
  return (
    <div className="py-6 space-y-6">
      ...
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold text-amber-400">タイムアタックモード</h1>
        <div className="flex justify-center space-x-8">
          <div className="text-center">
            <p className="text-sm text-purple-300">残り時間</p>
            <p className={`text-2xl font-bold ${getTimeColor()}`}>{timeLeft}秒</p>
          </div>
          <div className="text-center">
          ...
          <div className="text-center">
            <p className="text-sm text-purple-300">ラウンド</p>
      ...
      {/* ゲームオーバーダイアログ */}
      <Dialog open={showGameOverDialog} onOpenChange={handleGameOverDialogClose}>
        <DialogContent className="bg-gray-900 border-red-700">
          <DialogHeader>
            <DialogTitle className="text-2xl text-red-400">タイムアップ！</DialogTitle>
            <DialogDescription className="text-purple-300">時間切れです</DialogDescription>
          </DialogHeader>
          <div className="text-center space-y-4">
            <p className="text-white">見つけたペア: {matchedPairs} / 8</p>
            <p className="text-white">スコア: {score}点</p>
            <p className="text-white">手数: {moves}回</p>
            <div className="flex justify-center space-x-4">
              <Button onClick={resetGame} className="bg-purple-700 hover:bg-purple-600">
                もう一度
              </Button>
              <Button onClick={handleGameOverDialogClose} className="bg-amber-600 hover:bg-amber-700">
                スコアを記録
              </Button>
            </div>
          </div>
